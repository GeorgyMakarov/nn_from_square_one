...
#' 
#' Computes back propagation for neural net with dropout. The dropout matrices
#' come from forward propagation cache. The function does not apply dropout to
...
#' @export
back_propagation_dropout <- function(x, y, param, cache, l, activation, kp){
  ...
  for (i in seq(length(l) - 1, 1)){
    ...
    # Compute dropout for activation function of layers less than last layer
    # Do not compute d for i < 1, because we do not have it in cache list. Plus
    # it is not required for back propagation.
    if (i > 1){
      d   <- cache[[paste0('d', i - 1)]]
      dal <- dal * d
      dal <- dal / kp
    }
  ...
  rm(x, y, param, cache, l, activation, m, last_a, al, dal1, dal2, dal,
     cond, i, a_prev, w, z, actif, li, dz, dw, db, d, kp)
